...
function groupActivitiesByUser(activities: TActivityWithUser[]): TGroupedUserActivities[] {
    const userActivitiesMap = activities.reduce((acc, activity) => {
...
function calculateUserStatsAndSort(userGroupedActivities: TGroupedUserActivities[]): TGroupedUserActivitiesWithStats[] {
    const userActivitiesWithStats = userGroupedActivities.map(userGroup => {
        ...
        const totalMovingTime = validActivities.reduce((sum, activity) => sum + activity.movingTime, 0);
